...
def print_mean_accuracy(accs: np.ndarray, task_number: int,
    ...
    if joint:
        ...
        if setting == 'domain-il' or setting == 'general-continual':
            ...
            _, mean_acc_taskIl = mean_acc
            out_str = '{}: \t [Task-IL]: {} %'.format(prefix, round(mean_acc_taskIl, 2))
            print(out_str, file=sys.stderr)
            logging.info(out_str)
            print('\tRaw accuracy values: Task-IL {}'.format(accs[1]), file=sys.stderr)
            logging.info('\tRaw accuracy values: Task-IL {}'.format(accs[1]))
    else:
        ...
        if setting == 'domain-il' or setting == 'general-continual':
            ...
            _, mean_acc_taskIl = mean_acc
            out_str = '{} for {} task(s): \t [Task-IL]: {} %'.format(prefix, task_number, round(mean_acc_taskIl, 2))
            print(out_str, file=sys.stderr)
            logging.info(out_str)
            print('\tRaw accuracy values: Task-IL {}'.format(accs[1]), file=sys.stderr)
            logging.info('\tRaw accuracy values: Task-IL {}'.format(accs[1]))
